def main():
    ...
    if not is_sus():
        ...
        def choice_pizza(label_print):
            ...
            import json, re
            with open('data.json', 'r', encoding='utf-8') as data: user = json.load(data)
            ...
            while True:
                while (
                    not my_name.istitle() or my_name == ''
                ): my_name = input('Имя(C заглавной буквы): ')
                while (
                    not surname.istitle() or surname == ''
                ): surname = input('Фамилия(C заглавной буквы): ')
                while (
                    not patronymic.istitle() or patronymic == ''
                ...
                print(
                    f'\nФИО: {surname} {my_name} {patronymic}\n'
                    f'Email: {email}\n'
                    f'Номер телефона: {phone_num}\n'
                    f'Ваш год рождения: {my_year}\n'
                )
                user = {
                    "email": email,
                    "phone": phone_num,
                ...
                # indent = 4 - колличество пробелов на новых строках
                # ensure_ascii = False - сохранение неизвестных символов в исходном виде, а не ASCII
                with open('data.json', 'w', encoding='utf-8') as data: json.dump(user, data, indent=4, ensure_ascii=False)

                is_all_true = input('Все правильно?(Yes/No): ')
                if is_all_true == 'Yes' or is_all_true == 'Y' or is_all_true == 'y': break
                else:
                    surname = ''
                    patronymic = ''
                    my_name = ''
            ...
            years = (current_year, 18)
            is_18_years = lambda years: years[0] - years[1]
            if my_year <= is_18_years(years): running = 1
            ...
            while running:
                ...
                if running == 1: label_print(screen, f"Размер пиццы: < {size}м >", (400, 300), (255, 255, 0), True)
                    ...
                    if pizza_type == 5:
                        ...
                        if pizza_total[size - 1][5] == 0.5: label_print(screen, "    Кастомное название: \"Пицца 4 хлеба\"", (30, 240), (0, 255, 150), False)
                    if payment_type == 1: label_print(screen, f"Сдача: {round(money - pizza_total[size - 1][pizza_type], 2)}$", (30, 270), (0, 255, 255), False)
        ...
        async def funk0(): await asyncio.gather(goodbye_print(),thank_you())
        asyncio.run(funk0())
        sys.exit()

    else: print("Программа запущена с правами администратора (Windows) или root (Linux)! КАЗНИТЬ, НЕЛЬЗЯ ПОМИЛОВАТЬ!!!")

if __name__ == "__main__": main()
else: print("Этот файл импортирован как модуль! Что вы хотите сделать?! Я могу тогда вам предложить удалить system32 или сделать rm -rf /")
